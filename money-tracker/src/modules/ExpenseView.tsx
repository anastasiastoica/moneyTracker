...
import { ExpenseTransaction, Transaction } from './HomePage';
import { useTokenState } from './TokenContext';



function ExpenseView(props: {
  onSave: (t: ExpenseTransaction) => void,
  onHide: () => void,
  className: string,
  show: boolean
}) {
  ...
  <Button className={clazz} variant = "primary" onClick={() => setSelectedTransactionTypeId(transactionType.id!)}><img className='small-icon' src={transactionType.icon}/></Button>
    ...
    return (
      ...
      <div className='model-body'>
        <Modal.Body>
          <h5><input value={sum} onChange={e =>setSum(e.target.value)} type="number" placeholder='Add the amount'/></h5>
          ...
          <h5><FontAwesomeIcon icon={faNoteSticky}/>&nbsp;&nbsp;&nbsp;&nbsp;<input value={note} onChange={e =>setNote(e.target.value)} className="w3-input w3-animate-input" type="text" placeholder = "Write a note" style={{width:"135px"}}/></h5>
        ...
        <Modal.Footer>
          ...
          <Button onClick={() => props.onSave({sum: parseFloat(sum), date:date, note:note, category: selectedTransactionTypeId})}>Save</Button>
        </Modal.Footer>
      </Modal>
